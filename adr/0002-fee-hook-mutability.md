# ADR 0002: Fee hook ships at Phase 0, as mutable storage with a hard ceiling
...
properties, implemented in the Phase 0 scaffold:
...
4. **Ships initialized to 0.** The Phase 0 scaffold enforces the ceiling but
   does not yet gate the setter behind the timelock contract — that wiring
   is Phase 4, once `contracts/timelock` exists and is integrated.
...
against other revenue) lives in `refluo-prd-unified.md` §12 (local, not
committed). This ADR only documents the mechanism, which is already public
once `contracts/risk-engine` ships — the ceiling and the storage design are
visible in the contract source and on-chain, so there's nothing to protect
by keeping this ADR out of the repo.

## Consequences

- Once the timelock is wired in (Phase 4), `set_fee_bps()` must move off
  the exempt list — raising a fee is risk-increasing to the customer and
  gets the 24h delay + propose event, not the fast path pause/resume gets.
